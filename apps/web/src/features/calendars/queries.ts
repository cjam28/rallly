...
export async function getCalendarDashboardData(params: {
  ...
  const errorCutoff = new Date(Date.now() - 24 * 60 * 60 * 1000);
  ...
  const [
    ...
    prisma.availabilitySource.findMany({
  ...
  const activeSourceIds = new Set([
    ...
    ...icsSources.map((s) => s.id),
  ]);

  const visibleSyncStates = syncStates.filter(
    (s) =>
      s.lastStatus === "error" &&
      activeSourceIds.has(s.sourceId) &&
      s.lastSyncAt != null &&
      s.lastSyncAt >= errorCutoff,
  );

  return {
    cachedEvents,
    syncStates: visibleSyncStates,
    pollOptions,
    scheduledEvents,
  };
}
